...
class VIN(tf.keras.Model):
    def __init__(self, args):
        ...
        self.k = args.k # Number of Value Iteration computations
        self.ch_i = args.ch_i # Channels in input layer
        self.ch_h = args.ch_h # Channels in initial hidden layer
    ...
    def call(self, X, S1, S2):
        ...
        for _ in range(self.k-1):
            rv = tf.concat([r, v], axis=3)
            q = self.l_q(rv)     # TODO: check whether neeed to create a new layer
            v = tf.math.reduce_max(q, axis=3, keepdims=True, name='v')
        ...
        q_out = self.attention(q, S1, S2)

        logits = self.fcl(q_out)

        return logits
    
    def attention(self, tensor, S1, S2):
